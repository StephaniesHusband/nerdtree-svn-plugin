" ============================================================================
" File:        svn_status.vim
" Description: plugin for NERD Tree that provides svn status support
...
function! g:NERDTreeSvnStatusRefresh()
    ...
    let l:statusesStr = system('cd "' . l:root . '" && ' . l:svncmd)
    let l:statusesSplit = split(l:statusesStr, '\n')
    if l:statusesSplit != [] && l:statusesSplit[0] =~# 'warning: W155007:.*'
        let l:statusesSplit = []
        return
    endif
...
function! s:NERDTreeGetFileSvnStatusKey(us, them)
    if a:us ==# '?' && a:them ==# '?'
        return 'Untracked'
...
function! s:AddHighlighting()
    let l:synmap = {
                ...
                \ 'NERDTreeSvnStatusStaged'      : s:NERDTreeGetIndicator('Staged'),
